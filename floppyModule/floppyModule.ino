...
void recvWithEndMarker() {
    ...
    while(Serial.available() > 0 && newData == false) {
        ...
        if (rc != endMarker) {
           ...
           if(parsed / 100000 == 999){
              pitchWheel = parsed - 99900000;
              notePeriod = map(pitchWheel, 0, 16384, note_periods[note - 1], note_periods[note + 1]); //if the serial data starts with 999, it's a pitchwheel command, so receive it and map it to a period value
            }
            else if(parsed == 1024){
