...
void recvWithEndMarker() {
    ...
    while(Serial.available() > 0 && newData == false) {
        ...
        if (rc != endMarker) {
            ...
            if(parsed == 1024){
            ...
            else if(parsed / 1000000 == 999){
              pitchWheelInfo = parsed - 999000000;
              ...
              notePeriods[pitchWheelPin] = map(pitchWheel[pitchWheelPin], 0, 16384, note_periods[note - 1], note_periods[note + 1]); //if we get a pitchwheel command (999 + 0-16384 pitchwheel value + pin number), account for it in the note period. 8192 is no pitchbend
            }
